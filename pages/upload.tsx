...
const Upload = () => {
  ...
  const uploadVideo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileTypes.includes((selectedFile as File).type)) {
      client.assets
    ...
    <div className="flex w-full h-full absolute left-0 top-[60px] mb-10 pt-10 lg:pt-20 bg-[#F8F8F8] justify-center">
      <div className="bg-white rounded-lg xl:h-[80vh] flex gap-6 flex-wrap justify-between items-center p-14 pt-6 w-[80%]">
        <div>
          ...
          <div className="border-dashed rounded-xl border-4 border-gray-200 flex flex-col justify-center items-center outline-none mt-10 w-[260px] h-[460px] p-10 cursor-pointer hover:border-red-300 hover:bg-gray-100 transition">
            {isLoading ? (
              <p>Uploading...</p>
            ) : (
                ...
                {videoAsset ? (
                  <div className="h-full">
                    <video
                      ...
                      loop
                      controls
                      className="rounded-xl bg-black h-full"
                    ></video>
                  </div>
                ) : (
